        ...
        card(
          ...
          card_body(
            ...
            layout_column_wrap(
              ...
              uiOutput(ns("kpi_progresso_medio_2_ses")),
              
              uiOutput(ns("kpi_sad_ses")),
              uiOutput(ns("kpi_outros_ses")),
              uiOutput(ns("kpi_pge_ses")),
            ...
            DT::DTOutput(ns("tbl_status_geral"))
          )
        )
      ) #,
...
server_Safety <- function(id) {
  ...
  "))
    ...
    output$kpi_sad <- renderUI({
        ...
        total_geral <- rct_centralizados_sad() %>%
          dplyr::mutate(
            status_cod_chr = as.character(status_cod),
            status_cod_chr = dplyr::case_when(
              status == "04.1 Elaboração de Termo de Referência /ETP (SAD)" ~ "4.1",
   ...
   output$kpi_progresso_medio <- renderUI({
    ...
    output$kpi_progresso_medio_2_ses <- renderUI({
      df_info <- rct_unificadas_ses() %>%
        dplyr::mutate(
          progress_pct = round(pmin(100, pmax(0, as.numeric(status_cod) / 15 * 100)), 2)
        ) %>%
    ...
    output$kpi_sad_ses <- renderUI({
      tot_sad <- rct_unificadas_ses() %>%
        dplyr::filter(!is.na(status)) %>%
        dplyr::mutate(
          status_cod_chr = as.character(status_cod),
          status_cod_chr = dplyr::case_when(
      ...
      total_geral <- rct_unificadas_ses() %>%
        dplyr::mutate(
          status_cod_chr = as.character(status_cod),
          status_cod_chr = dplyr::case_when(
            status == "04.1 Elaboração de Termo de Referência /ETP (SAD)" ~ "4.1",
    ...
    output$kpi_outros_ses <- renderUI({
      tot_ses <- rct_unificadas_ses() %>%
        dplyr::filter(!is.na(status)) %>%
        dplyr::mutate(
          status_cod_chr = as.character(status_cod),
          status_cod_chr = dplyr::case_when(
    ...
    output$kpi_pge_ses <- renderUI({
      tot_pge <- rct_unificadas_ses() %>%
        dplyr::filter(!is.na(status)) %>%
        dplyr::mutate(
          status_cod_chr = as.character(status_cod),
          status_cod_chr = dplyr::case_when(
      ...
      output$tbl_centralizadas_sad <- DT::renderDT({
        ...
        df_view <- df_info |>
          dplyr::mutate(
           ...
           progress_pct = if_else(
             is.na(progress_pct), "-",
             sprintf("
        <div style='display:flex;flex-direction:column;align-items:center;width:100%%;'>
          <div style='width:100%%;background:#e9ecef;border-radius:9999px;height:12px;overflow:hidden;margin-bottom:4px;'>
            <div style='height:100%%;width:%1$.2f%%;
          ...
          <div style='font-size:13px;color:#004C6D;font-weight:500;' data-order='%1$.4f'>%1$.2f%%</div>
        </div>",
                     progress_pct,
                     # clareia no início e escurece no final
                     75 - (progress_pct / 2),
                     55 - (progress_pct / 3)
             )
           )
    ...
    ")
      ...
      output$tbl_status_geral <- DT::renderDT({
        df_info <- rct_unificadas_ses() |>
          dplyr::select(
            is_ses_unificadas, sei, grupo, data_de_inicio, status, observacoes,
            atraso, status_cod, progress_pct, tempo_na_fase
