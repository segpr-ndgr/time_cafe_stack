...
safe_get <- function(lst, key) {
  if (!is.null(lst) && key %in% names(lst)) {
    val <- lst[[key]]
...
# código novo
# Determina os anos reais vindos do server (últimos 3 anos disponíveis)
anos_relatorio <- sort(as.integer(gsub("ano", "", names(params$totais_vetor))))

# Totais
total_m2    <- safe_get(params$totais_vetor, paste0("ano", anos_relatorio[1]))
total_m1    <- safe_get(params$totais_vetor, paste0("ano", anos_relatorio[2]))
total_atual <- safe_get(params$totais_vetor, paste0("ano", anos_relatorio[3]))

# Médias
media_m2    <- safe_get(params$medias_vetor, paste0("ano", anos_relatorio[1]))
media_m1    <- safe_get(params$medias_vetor, paste0("ano", anos_relatorio[2]))
media_atual <- safe_get(params$medias_vetor, paste0("ano", anos_relatorio[3]))

# Cria as chaves
# keys <- paste0("ano", c(ano - 2, ano - 1, ano))
# 
# # Totais
# total_m2    <- safe_get(params$totais, keys[1])
# total_m1    <- safe_get(params$totais, keys[2])
# total_atual <- safe_get(params$totais, keys[3])
# 
# # Médias
# media_m2    <- safe_get(params$medias, keys[1])
# media_m1    <- safe_get(params$medias, keys[2])
# media_atual <- safe_get(params$medias, keys[3])
...
**Período do Relatório de Consumo**
`r paste0(min(anos_relatorio), " a ", max(anos_relatorio))`
...
# df <- params$tbl_mes
# 
# # Define colunas fixas e possíveis colunas de mês
# fixed_cols <- c("ano", "nome_programa", "descricao_produto", "nome_unidade_medida")
# month_cols <- c("jan", "fev", "mar", "abr", "mai", "jun", 
#                 "jul", "ago", "set", "out", "nov", "dez")
# 
# if (!is.null(df) && nrow(df) > 0) {
#   
#   # Identifica colunas de mês que existem no df
#   existing_month_cols <- intersect(month_cols, names(df))
#   
#   # Substitui NA apenas nas colunas que existem
#   df_flex <- df %>%
#     mutate(across(all_of(existing_month_cols), ~replace_na(.x, 0))) %>%
#     select(any_of(c(fixed_cols, existing_month_cols)))
#   
#   # Cria labels apenas para as colunas presentes
#   label_map <- c(
#     ano = "Ano",
#     nome_programa = "Unidade",
#     descricao_produto = "Descrição",
...
#   )
#   
#   # Aplica labels apenas às colunas do df_flex
#   flextable(df_flex) %>%
#     set_header_labels(values = label_map[names(df_flex)]) %>%
#     fontsize(size = 9, part = "all") %>%
#     autofit()
#   
# } else {
...
#   )
#   
#   ft <- flextable(df_flex) %>%
#     set_header_labels(values = label_map[names(df_flex)]) %>%
#     fontsize(size = 9, part = "all") %>%
#     autofit()
#   
# } else {
#   ft <- "Tabela de consumo não disponível."
# }
# 
